...
{
    m_timer.setInterval(GlobalFun::getProperty("config", "camera", "interval").toInt());
    ...
    try {
        ...
        if ( ret == MV_OK ) {
            if ( pstDevList.nDeviceNum == 0 ) {
                ...
                if ( ret != MV_OK ) { emit error("CreateHandle failed !"); return; }
                ret = MV_CC_OpenDevice(m_devHandle);                            // 打开设备
                if ( ret != MV_OK ) { emit error("OpenDevice failed !"); return; }
